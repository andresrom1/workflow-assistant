<template>
  <MangoLayout hide-header>
    <Head>
      ...
      <meta
        name="description"
        content="Te escribimos antes de que pase nada. Cotizás en un chat, y cuando algo se rompe, te atiende una persona: tu productor, siempre el mismo."
      />
    </Head>
    ...
    <!-- ─── Hero forma-carta (invita a leer, sin presumir el choque) ─────── -->
    <section class="relative overflow-hidden px-5 pt-16 pb-16 sm:pt-24 sm:pb-20">
      <h1 class="sr-only">MANGO — IA que cotiza, humano que te cuida. Seguro de auto en Argentina.</h1>

      <div class="max-w-2xl mx-auto relative">
        <p v-reveal class="font-mono text-xs uppercase tracking-[0.22em] text-right mb-10" :style="{ color: 'var(--mg-fg-faint)' }">
          Buenos Aires, {{ fechaCarta }}
        </p>

        <p v-reveal="60" class="mg-display text-5xl sm:text-6xl mb-9">Hola.</p>

        <div v-reveal="140" class="space-y-6 text-lg sm:text-xl leading-[1.7]" :style="{ color: 'var(--mg-fg)' }">
          <p>
            Te escribimos antes de que pase nada. Porque de los seguros nadie se acuerda hasta el día
            del choque, del robo, del granizo — y ese día, lo último que querés es un 0800, un formulario
            y gente que te pide los mismos datos una y otra vez.
          </p>
          <p>
            <em class="italic" :style="{ color: 'var(--mg-mango)' }">Nosotros lo pensamos al revés.</em>
            La IA se encarga de la parte aburrida: cotizar, comparar, repetir. Y una persona —tu productor,
            siempre el mismo— se queda para lo que importa: el día en que algo se rompe.
          </p>
        </div>

        <div v-reveal="240" class="mt-10 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <a :href="primaryCta.href" :target="primaryCta.target" class="mg-btn-primary !px-9 !py-4 !text-sm text-center">
            {{ primaryCta.label }}
          </a>
          <a :href="secondaryCta.href" :target="secondaryCta.target" class="mg-btn-ghost !px-9 !py-4 !text-sm text-center">
            {{ secondaryCta.label }}
          </a>
        </div>

        <div v-reveal="320" class="mt-10 flex items-center gap-4">
          <MangoFruit :outline="true" class="w-8 h-8 shrink-0" />
          <div class="flex-1 h-px" :style="{ background: 'var(--mg-hairline)' }"></div>
          <p class="mg-folio" :style="{ color: 'var(--mg-fg-faint)' }">Seguí leyendo</p>
        </div>
      </div>
    </section>
    ...
    <section class="relative overflow-hidden px-5 py-28">
      ...
      <div class="max-w-2xl mx-auto text-center relative">
        ...
        <p v-reveal="360" class="mt-10 font-serif italic text-base" :style="{ fontFamily: 'var(--mg-font-display)', color: 'var(--mg-fg-dim)' }">
          — Andrés, tu productor
        </p>
      </div>
    </section>
