<template>
  <MangoLayout hide-header>
    <Head>
      <title>MANGO — El seguro, reescrito.</title>
      <meta
        name="description"
        content="Cotizás en un chat. Y cuando algo pasa de verdad, te atiende una persona: tu productor, siempre el mismo, por WhatsApp."
      />
    </Head>

    <!-- ─── Masthead (revista, no glassmorphism) ───────────────────────── -->
    <header
    ...
    <!-- ─── Hero editorial (una columna, tipográfico) ──────────────────── -->
    <section class="relative overflow-hidden">
      <!-- Mango como línea (outline): reconocible, sin tapar el titular -->
      ...
      <div class="max-w-3xl mx-auto px-5 pt-24 pb-20 sm:pt-32 sm:pb-28 relative text-center">
        ...
        <p v-reveal="320" class="mt-6 mg-folio" :style="{ color: 'var(--mg-fg-faint)' }">Sin llamados · Sin vueltas · Gratis</p>
      </div>
    </section>

    <!-- ─── Escena I · El chat (foto de revista, no producto flotante) ───── -->
    ...
    <!-- ─── Capítulo · El modelo, al revés (pull-quote editorial) ────────── -->
    <section class="px-5 py-24">
      <div class="max-w-3xl mx-auto">
        <p v-reveal class="mg-folio mb-8 text-center">El modelo, al revés</p>
        <blockquote v-reveal="80" class="pull-quote mg-display text-3xl sm:text-[2.6rem] leading-[1.12] text-center">
          Los seguros siempre fueron al revés.
          <span :style="{ color: 'var(--mg-mango)' }">Nosotros los dimos vuelta.</span>
        </blockquote>
        <p v-reveal="160" class="mt-9 mx-auto max-w-lg text-center leading-relaxed" :style="{ color: 'var(--mg-fg-dim)' }">
        ...
        <!-- Spread asimétrico: "antes" chico y apagado / "ahora" grande -->
    ...
    <!-- ─── Capítulo · La app, como índice de revista ────────────────────── -->
    ...
    <!-- ─── Postventa 100% humana (cálida, mango-tint, no estridente) ────── -->
    <section class="relative overflow-hidden px-5 py-24" :style="{ background: 'var(--mg-mango-tint)' }">
      ...
      <div class="max-w-4xl mx-auto relative">
        <div class="text-center mb-16">
          <p v-reveal class="mg-folio mb-6" :style="{ color: 'var(--mg-mango)' }">Postventa 100% humana</p>
          <h2 v-reveal="80" class="mg-display text-4xl sm:text-6xl leading-[1.04] mb-6">
            Cuando algo pasa,<br>te atiende una persona.
          </h2>
          <p v-reveal="160" class="mx-auto max-w-lg leading-relaxed" :style="{ color: 'var(--mg-fg-dim)' }">
        ...
        <div class="grid lg:grid-cols-2 gap-10 lg:gap-14 items-center">
          <div>
            <p v-reveal class="text-sm font-semibold mb-5" :style="{ color: 'var(--mg-fg)' }">Con MANGO nunca te vas a topar con:</p>
            ...
            <div v-reveal="120" class="mg-card p-6" :style="{ borderLeft: '3px solid var(--mg-mango)' }">
              <p class="font-semibold mb-1.5" :style="{ color: 'var(--mg-fg)' }">En su lugar: una persona real.</p>
          ...
          <figure v-reveal="120">
            <div ref="pasChatEl" class="chat-frame mg-card p-5" :style="{ background: 'var(--mg-surface)' }">
              <div class="flex items-center gap-3 pb-4 mb-4" :style="{ borderBottom: '1px solid var(--mg-hairline)' }">
            ...
            <figcaption class="mt-4 text-center mg-folio" :style="{ color: 'var(--mg-fg-faint)' }">
              Fig. 2 — Una persona real, el día que más lo necesitás.
            </figcaption>
          </figure>
        </div>
      </div>
...
// ── v-reveal (idéntico a la v1: chequeo por rect + observer opcional) ────
...
// perfecta de librería. Cada una lleva un punto/acento en naranja de marca.
const appFeatures = [
  {
...
<style scoped>
/* Folio tipo revista (mono, tracking amplio) — reemplaza el overline SaaS. */
.mg-folio {
...
/* Subrayado a mano bajo la línea mango del hero: se dibuja al revelarse. */
.hero-ink {
  position: relative;
...
/* Pull-quote editorial con regla mango a la izquierda. */
.pull-quote {
  position: relative;
}

/* Card de chat: sombra cálida en capas (foto de revista), sin anillo de IA. */
.chat-frame {
  box-shadow:
...
/* Icono a mano: fila del índice; el trazo se tiñe de mango al hover. */
.hand-icon {
  display: inline-flex;
  width: 40px;
  height: 40px;
...
/* Mango del hero: bob suave con presencia real (no fantasma). */
.hero-fruit {
  opacity: 0.8;
...
/* Tachado que se dibuja al entrar (lista "nunca"). */
.strike-text::after {
  content: '';
  position: absolute;
  left: 0;
...
/* Enlace-CTA del masthead (editorial, subrayado en hover). */
.mg-link-cta {
