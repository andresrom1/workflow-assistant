...
class AgentToolAdapter implements AIProviderAdapterInterface
{
    ...
    public function handleToolCall(array $payload, string $toolName): array
    {
        ...
        $this->logAdapter("Tool llamado: {$toolName}", [
            'payload' => $payload,
     ...
     * @param  Conversation  $conversation  La conversación asociada al thread_id
     * @return array Respuesta formateada para la IA
     */
    ...
    public function identifyVehicle(array $data, Conversation $conversation): array
    {
        ...
        if (! $vehicle->wasRecentlyCreated) {
            ...
            $this->logAdapter('Vehículo actualizado/transferido.', ['patente' => $vehicle->patente]);
        }
    ...
    public function getQuote(array $data): array
    {
        ...
        $alternatives = $quote->alternatives->map(fn (QuoteAlternative $alt) => [
            ...
            // external_code y external_quote_id excluidos — scope de proveedor (ver quote_provider_refs)
        ]);
    ...
    /**
     * Valida que el payload contenga los campos comunes a todos los tools.
     *
     * @param  array  $data  El payload a validar
     *
    ...
    /**
     * Normaliza el payload del proveedor (OpenAI) al contrato interno del adapter.
     * Los alias específicos por tool (preference, quoteId) se mapean aquí;
     * cada tool validará si los necesita.
     *
     * @param  array  $payload  El payload original del proveedor
     ...
     * Lanza InvalidArgumentException si la validación falla.
     *
    ...
    private function tryResolveQuoteById(?int $quoteId): bool
    {
        ...
        try {
            ...
            if (! $quote) {
                $this->logAdapter('Quote ya no está pendiente, se omite resolución.', ['quote_id' => $quoteId]);
    ...
    /**
     * Construye una respuesta de éxito estándar para la IA.
     */
    ...
    /**
     * Construye una respuesta de error estándar para la IA.
     */
