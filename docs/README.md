...
| 04 | [04-ingesta-local-documentos.md](v3/04-ingesta-local-documentos.md) | **(Implementado, Fases 1–3)** Ingesta local: un script Python (`ingestor/`) detecta PDFs de pólizas en la PC, extrae datos sin LLM y los manda (JSON+PDF) a `POST /api/ingesta/documentos` (Sanctum). El server estaciona en `ingested_documents` (dedup por hash + PDF en R2) y, con confirmación humana en `/ingesta-pendientes`, materializa `Customer→Risk→Poliza→PolicyDocument` (vía dedup), agrupando por `numero_poliza` con fallback a `patente`. La contraparte server está documentada para el cliente en `ingestor/app/v5/SERVIDOR.md`. |
