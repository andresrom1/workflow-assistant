...
| V2-6 | Retiro de legacy PAS | 🚧 | **pas_mobile extirpado** (rama `cirugia-v2`): `MobileAppQuoteResolution`, `QuoteOfferedToPas`, `FallbackToApiListener`, `QuoteWebhookController` + ruta `/webhooks/quote-update`, `MobileSyncLog` + columnas `mobile_*` de `quotes`, settings/config `mobile_app.*`, badges `offered_pas`/`rejected_pas`. `QuoteService` colapsado a estrategia única (`api`). **Pendiente:** borrar stub `n8n-whatsapp/` (dead, no es el puente WhatsApp). |
